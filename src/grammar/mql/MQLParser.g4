...
root:
    setStatement? (matchQuery | describeQuery | insertQuery) EOF;

matchQuery:
    matchStatement projectSimilarity? whereStatement? groupByStatement? orderByStatement?
        returnStatement;

projectSimilarity:
    K_PROJECT_SIMILARITY '(' VARIABLE ',' VARIABLE ',' STRING ',' (
        fixedNodeInside
        | tensor
    ) ',' metricType ')';
...
insertLabelList:
    K_LABEL insertLabelElement (',' insertLabelElement)*;

insertPropertyList:
    K_PROPERTY insertPropertyElement (',' insertPropertyElement)*;

insertEdgeList:
    K_EDGE insertEdgeElement (',' insertEdgeElement)*;

insertLabelElement: '(' (identifier | ANON_ID) ',' STRING ')';

insertPropertyElement:
    '(' fixedNodeInside ',' STRING ',' value ')';

// FROM, TO, TYPE
insertEdgeElement:
    '(' fixedNodeInside ',' fixedNodeInside ',' identifier ')';
...
returnStatement:
    K_RETURN K_DISTINCT? returnItem (',' returnItem)* (
        K_LIMIT UNSIGNED_INTEGER
    )?                                                     # returnList
...
similaritySearch:
    K_SIMILARITY_SEARCH '(' VARIABLE ',' VARIABLE ',' STRING ',' (
        fixedNodeInside
        | tensor
    ) ',' ('+' | '-')? UNSIGNED_INTEGER ')';
